...
struct DenseMat {
    ...
    DenseMat() = default;
    explicit DenseMat(int n_) : n(n_), a((size_t)n_*(size_t)n_, 0.0) {}

    inline double& operator()(int i, int j) { return a[(size_t)i*(size_t)n + (size_t)j]; }
    inline double  operator()(int i, int j) const { return a[(size_t)i*(size_t)n + (size_t)j]; }

    void release() { a.clear(); a.shrink_to_fit(); n = 0; }
...
inline std::vector<double> matvec(const DenseMat& A, const std::vector<double>& x) {
...
inline DenseMat transpose(const DenseMat& A) {
    DenseMat T(A.n);
    for (int i=0;i<A.n;++i) for (int j=0;j<A.n;++j) T(j,i) = A(i,j);
    return T;
}

inline void transpose_inplace(DenseMat& A) {
    const int n = A.n;
    for (int i = 0; i < n; ++i)
        for (int j = i + 1; j < n; ++j)
...
inline bool cholesky_lower_inplace(DenseMat& A) {
    const int n = A.n;
    for (int i=0;i<n;++i) {
        for (int j=0;j<=i;++j) {
            double sum = A(i,j);
...
inline void solve_lower(const DenseMat& L, const std::vector<double>& b, std::vector<double>& x) {
    const int n = L.n;
...
inline void solve_upper_from_lower_transpose(const DenseMat& L, const std::vector<double>& b, std::vector<double>& x) {
    const int n = L.n;
    x.assign((size_t)n, 0.0);
    for (int i=n-1;i>=0;--i) {
        double s = b[(size_t)i];
...
inline DenseMat solve_lower_many(const DenseMat& L, const DenseMat& B) {
    assert(L.n == B.n);
    const int n = L.n;
    DenseMat X(n);
    for (int j=0;j<n;++j) {
...
inline void solve_lower_many_inplace(const DenseMat& L, DenseMat& B) {
    assert(L.n == B.n);
    const int n = L.n;
    for (int j = 0; j < n; ++j) {
        for (int i = 0; i < n; ++i) {
...
inline DenseMat solve_upper_from_lower_transpose_many(const DenseMat& L, const DenseMat& B) {
    assert(L.n == B.n);
    const int n = L.n;
    DenseMat X(n);
    for (int j=0;j<n;++j) {
...
inline void solve_upper_from_lower_transpose_many_inplace(const DenseMat& L, DenseMat& B) {
    assert(L.n == B.n);
    const int n = L.n;
    for (int j = 0; j < n; ++j) {
        for (int i = n - 1; i >= 0; --i) {
...
inline SymEig jacobi_symmetric_eig(DenseMat A, int max_sweeps=200, double tol=1e-12) {
    const int n = A.n;
...
inline GenSymEig generalized_selfadjoint_eig_inplace(DenseMat& K, DenseMat& M) {
    assert(K.n == M.n);
    const int n = K.n;
...
inline GenSymEig generalized_selfadjoint_eig(const DenseMat& K, const DenseMat& M) {
    DenseMat K_copy = K;
    DenseMat M_copy = M;
    return generalized_selfadjoint_eig_inplace(K_copy, M_copy);
}

inline DenseMat dense_matmul(const DenseMat& A, const DenseMat& B) {
    assert(A.n == B.n);
    const int n = A.n;
    DenseMat C(n);
    for (int i=0;i<n;++i) {
...
inline std::vector<double> dense_matvec(const DenseMat& A, const std::vector<double>& x) {
    return matvec(A, x);
}

inline DenseMat dense_At_B(const DenseMat& A, const DenseMat& B) {
    // A^T * B
    assert(A.n == B.n);
    const int n = A.n;
    DenseMat C(n);
...
inline std::vector<double> dense_At_vec(const DenseMat& A, const std::vector<double>& x) {
    assert((int)x.size()==A.n);
    const int n = A.n;
...
inline bool solve_spd_cholesky(DenseMat A_spd, const std::vector<double>& b, std::vector<double>& x) {
