#include "fractional_matrix_free_operator.h"
...
FractionalMatrixFreeP1Operator::FractionalMatrixFreeP1Operator(const FEMMesh& mesh, FractionalMatrixFreeOptions options)
    : mesh_(&mesh),
      options_(options),
      kernel_{.s = options.s, .scale = options.scale},
      nodal_mass_(build_fractional_nodal_mass(mesh))
{
...
Real FractionalMatrixFreeP1Operator::diagonal(Index i) const noexcept {
